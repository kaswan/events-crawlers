...
page = agent.get("https://www.ptotjinzaibank.com/ot/")
...
page.search('//div[@class="melon-box-white"]//a').select{|link| link && link[:href] && link[:href].match(/area/) }.each do |link|
  ...
  result_page.search('//div[@class="melon-resultBox_header"]').each do |dlink|
    ...
    if !header_text.match(/月/).nil? && !header_text.match(/日/).nil?
      ...
      ot = OtJinzaiBank.where(id: id.to_i)
      unless ot.first.nil?
        ot.first.update_columns(:calendar_date => header_text)
      end  
    end 
  end
  ...
  offer_page_links.each do |offer|
    ...
    begin 
      ...
      begin  
        OtJinzaiBank.transaction do  
          OtJinzaiBank.where(id: id.to_i).first_or_initialize.tap do |of| 
            of.id = id.to_i
